...
$$
\dot{\rho}\Lambda + 3H(1+w\Lambda)\rho_\Lambda = 0.
$$
...
Recompute $S(z)$ for the Euclid fiducial cosmology.
Perform a joint fit of ${w_0, w_a, \theta, \sigma_X}$.
Report either a detection or a tighter upper limit ($\sigma_X \lesssim 10^{-5}$).
...
$$
The field satisfies:
Spatial isotropy — $\Xi$ depends only on $r$, not on direction;
Microcausality — $\Xi = 0$ if $\Delta x^2 - (aH)^2 r^2 > 0$;
Fluctuation-dissipation balance — linear Langevin equation with damping $\theta$ and white noise $\sigma$.
...
$$
\langle X_{\mathbf{k}1}(x_1) X{\mathbf{k}_2}(x_2) \rangle = (2\pi)^3 \delta_D(\mathbf{k}_1 + \mathbf{k}_2) , P_X(x_1, x_2, k).
$$
...
$$
\mathcal{L}{\rm int} = -\frac{1}{4} B_F(\phi) , F{\mu\nu} F^{\mu\nu},
$$
